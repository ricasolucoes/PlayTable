...
def gerar_um(cliente, man, asset, tentativas):
    ...
    if not api_key:
        print("OPENAI_API_KEY não está no ambiente.")
        sys.exit(2)
        
    ultimo = None
    for tentativa in range(1, tentativas + 1):
        try:
            # Controle de gastos diarios ($1 max)
            spend_file = MANIFESTOS / ".daily_spend"
            hoje = datetime.now(timezone.utc).strftime("%Y-%m-%d")
            gasto_atual = 0.0
            if spend_file.exists():
                try:
                    dados_gasto = json.loads(spend_file.read_text())
                    if dados_gasto.get("date") == hoje:
                        gasto_atual = dados_gasto.get("spend", 0.0)
                except Exception:
                    pass
            
            custo_estimado = 0.08
            if gasto_atual + custo_estimado > 3.00:
                raise RuntimeError("Limite diário de $1.00 excedido! Abortando.")
            ...
            if "data" in data and len(data["data"]) > 0:
                ...
                if "url" in item:
                    ...
                    spend_file.write_text(json.dumps({"date": hoje, "spend": gasto_atual + custo_estimado}))
                    return img_response.content, modelo
                elif "b64_json" in item:
                    spend_file.write_text(json.dumps({"date": hoje, "spend": gasto_atual + custo_estimado}))
                    return base64.b64decode(item["b64_json"]), modelo
                else:
                    raise KeyError(f"Nem 'url' nem 'b64_json' na resposta: {item}")
            raise RuntimeError(f"resposta sem imagem: {data}")
        except Exception as e:
            ultimo = e
            print(f"      ...{type(e).__name__}; nova tentativa em {2**tentativa}s ({e})")
