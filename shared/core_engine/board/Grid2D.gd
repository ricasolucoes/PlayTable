## A generic 2D grid representation.
class_name Grid2D
extends RefCounted
...
func _init(p_rows: int = 0, p_cols: int = 0, default_value: Variant = null) -> void:
	rows = p_rows
	cols = p_cols
...
func get_cell(r: int, c: int) -> Variant:
	if not is_valid(r, c):
		return null
	return cells[r * cols + c]

func get_cell_pos(pos: Vector2i) -> Variant:
	return get_cell(pos.x, pos.y)

func set_cell(r: int, c: int, value: Variant) -> void:
	if is_valid(r, c):
		cells[r * cols + c] = value

func set_cell_pos(pos: Vector2i, value: Variant) -> void:
...
func clear(default_val: Variant = null) -> void:
...
func count_consecutive(start: Vector2i, dir: Vector2i, match_value: Variant) -> int:
	var count: int = 0
	var curr: Vector2i = start + dir
	while is_valid_pos(curr) and get_cell_pos(curr) == match_value:
		count += 1
		curr += dir
	return count

func count_streak_bidirectional(pos: Vector2i, dir: Vector2i, match_value: Variant) -> int:
	var count: int = 1 # A própria peça
	count += count_consecutive(pos, dir, match_value)
	count += count_consecutive(pos, -dir, match_value)
	return count

func find_all_matching(match_value: Variant) -> Array[Vector2i]:
	var result: Array[Vector2i] = []
	for r in range(rows):
		for c in range(cols):
...
func count_matching(match_value: Variant) -> int:
	var count: int = 0
	for item in cells:
		if item == match_value:
			count += 1
...
func is_full(empty_value: Variant = null) -> bool:
	for item in cells:
		if item == empty_value:
			return false
	return true
