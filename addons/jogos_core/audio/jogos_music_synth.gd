class_name JogosMusicSynth
extends RefCounted

## Música de fundo procedural renderizada em tempo de execução via síntese harmônica.
## Elimina dependência de assets de áudio para música de fundo leve em jogos.
## Extraído e generalizado de PlayTable/core/audio/MusicSynth.gd.
...
static func loop_samples(mood: String, bars: int = COMPASSOS, rate: int = RATE) -> int:
	var p: Dictionary = PARTITURAS.get(mood, PARTITURAS["menu"])
